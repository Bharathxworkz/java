<!DOCTYPE html>
<html lang="en">
<%@ page language="java" contentType="text/html; charset=US-ASCII" pageEncoding="US-ASCII" %>
<%@ page isELIgnored="false" %>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Registration Page</title>
    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <!-- Header Section -->
    <header class="bg-light shadow-sm">
        <div class="container d-flex justify-content-between align-items-center py-3">
                        <img src="data:image/png;base64," alt="Logo" style="height: 50px;">


            <nav class="d-flex align-items-center">
                <a href="signin" class="btn btn-primary me-3">
                    <i class="bi bi-person"></i>logout
                </a>
                <div class="d-flex align-items-center">
                    <img src="view/${dto.getFileName()}" alt="Profile" class="rounded-circle" style="width: 50px; height: 50px;">
                    <div class="ms-2">
                        <p class="mb-0">Hi, ${dto.getFirstName()}</p>
                    </div>
                </div>
            </nav>
        </div>
    </header>

    <div class="container mt-5">
        <h5 class="text-center text-danger">${update}</h5>
        <h2 class="text-center mb-4">Registration Form</h2>
        <div class="row justify-content-center">
            <div class="col-md-8">
                <form id="registrationForm" action="updated" method="post" enctype="multipart/form-data">

                    <div class="mb-3">
                        <label for="id" class="form-label">ID</label>
                        <input type="text" class="form-control" id="id" name="id" value="${dto.getId()}" readonly>
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${fname}</h5>
                        <label for="firstName" class="form-label">First Name</label>
                        <input type="text" class="form-control" id="firstName" name="firstName" value="${dto.getFirstName()}" placeholder="Enter your first name" required>
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${lname}</h5>
                        <label for="lastName" class="form-label">Last Name</label>
                        <input type="text" class="form-control" id="lastName" name="lastName" value="${dto.getLastName()}" placeholder="Enter your last name">
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${emailError}</h5>
                        <label for="email" class="form-label">Email</label>
                        <input type="email" class="form-control" id="email" name="email" value="${dto.getEmail()}" placeholder="Enter your email" required>
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${altEmailError}</h5>
                        <label for="alternateEmail" class="form-label">Alternate Email</label>
                        <input type="email" class="form-control" id="alternateEmail" name="alternateEmail" value="${dto.getAlternateEmail()}" placeholder="Enter your alternate email">
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${contactError}</h5>
                        <label for="contact" class="form-label">Contact Number</label>
                        <input type="tel" class="form-control" id="contact" name="contact" value="${dto.getContact()}" placeholder="Enter your contact number" required>
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${altContactError}</h5>
                        <label for="alternateContact" class="form-label">Alternate Contact Number</label>
                        <input type="tel" class="form-control" id="alternateContact" name="alternateContact" value="${dto.getAlternateContact()}" placeholder="Enter your alternate contact number">
                    </div>

                    <div class="mb-3">
                        <label for="dateOdBirth" class="form-label">Date of Birth</label>
                        <input type="date" class="form-control" id="dateOdBirth" name="dateOdBirth" value="${dto.getDateOdBirth()}">
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${placeError}</h5>
                        <label for="place" class="form-label">Place</label>
                        <input type="text" class="form-control" id="place" name="place" value="${dto.getPlace()}" placeholder="Enter your place">
                    </div>

                    <div class="mb-3">
                        <label for="file" class="form-label">Choose File</label>
                        <input type="file" class="form-control" id="file" name="file" value="${dto.getFileName()}">
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${stateError}</h5>
                        <label for="state" class="form-label">State</label>
                        <select class="form-control" id="state" name="state">
                            <option>${dto.getState()}</option>
                            <option value="Karnataka">Karnataka</option>
                            <option value="Maharashtra">Maharashtra</option>
                            <option value="Kerala">Kerala</option>
                        </select>
                    </div>

                    <div class="mb-3">
                        <h5 class="text-center text-danger">${districtError}</h5>
                        <label for="district" class="form-label">District</label>
                        <select class="form-control" id="district" name="district">
                            <option>${dto.getDistrict()}</option>
                            <option value="Bangalore">Bangalore</option>
                            <option value="Pune">Pune</option>
                            <option value="Kochi">Kochi</option>
                        </select>
                    </div>

                    <button type="submit" class="btn btn-primary w-100">Update</button>
                </form>
            </div>
        </div>
    </div>

    <!-- Footer Section -->
    <footer class="bg-light mt-5 py-3">
        <div class="container text-center">
            <p>&copy; 2025 Your Company Name. All rights reserved.</p>
        </div>
    </footer>

    <!-- Bootstrap JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
